...
> **You are on `main`** — the wire `version == 0x02` line, library `2.8.0`.
...
| Branch | Wire | Line |
| --- | --- | --- |
| `main` | `0x02` | Current development, library `2.8.0` — **this branch** |
...
| Release | `v2.8.0` | The git tag over spec, library and vectors. Library layer added without a wire change: `btp::messages` in `2.2.0`, its verbatim relay path in `2.3.0`, `btp::telemetry` (the `PACKED_LE` / `TLV_LE` sample body) in `2.4.0`, its body-only reader/writer mode in `2.5.0`, `btp::session` (`btp::DedupCache`) in `2.6.0`, `btp::endpoint` (`btp::Endpoint`) in `2.7.0`, `btp::receiver` (`btp::Receiver`) in `2.8.0` |
| Branch | `1.x` | Holds the previous major; `main` holds the newest |
| Library | `2.8.0` | `CMakeLists.txt`, `library.json`, `kLibraryVersion*` |
...
the `1.x` line published `v1.1.0-beta`, while `main` declares a plain `2.8.0`.
