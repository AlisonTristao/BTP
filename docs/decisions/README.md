...
| [0012 - Criptografia AEAD do payload (v2.0)](0012-criptografia-aead-payload.md) | Proposta | Flag `ENCRYPTED`, AES-128-GCM, sub-campo `CIPHER_ID` no wire, nonce de `source_id`/`boot_id`/`sequence`, AAD do header canonicalizado, chave fora do wire |
