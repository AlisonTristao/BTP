...
This is why the reference library's `btp::DedupCache`
([Using the library §13](library.md#13-the-session-layer)) is built with no
clock and no tie to session state: it is cleared only on an executor reboot.
