...
### 1.9 Communication over exposed RF channels

Wireless communication introduces an additional concern because the physical medium is inherently shared.

A device within radio range may be able to observe transmitted frames without requiring a physical connection to the system.

Depending on the underlying transport and its security mechanisms, a malicious device may attempt to:

* capture telemetry;
* observe commands and terminal traffic;
* modify transmitted data;
* inject forged frames;
* retransmit previously captured frames.

This is particularly relevant when the same channel carries both monitoring and control traffic.

Integrity mechanisms such as a CRC can detect accidental transmission errors, but they do not provide protection against intentional modification. An attacker can modify a payload and calculate a new valid CRC.

BTP therefore supports authenticated encryption for applications that require confidentiality and cryptographic integrity.

When encryption is enabled, the application payload is encrypted and authenticated using a shared cryptographic key.

A receiver can then verify that an encrypted payload was generated by a participant with access to the expected key and that the protected data was not modified during transmission.

Encryption also prevents a passive observer from directly reading the protected payload.

These mechanisms provide protection at the BTP message level and do not depend exclusively on security provided by the underlying communication technology.

They do not, however, solve every wireless security problem. In particular, authenticated encryption does not prevent:

* radio-frequency jamming;
* traffic analysis based on visible metadata;
* capture of encrypted frames;
* replay unless an additional anti-replay mechanism is provided.

The security properties and limitations of BTP encryption are defined separately in the encryption specification.
...
Traffic classes have different requirements for:
...
### 3.9 Authenticated payload protection

Communication links cannot always be assumed to provide sufficient confidentiality or authenticity.

BTP can protect application payloads using authenticated encryption.

The mechanism provides two main properties:

* **confidentiality**, preventing an observer without the key from directly reading the protected payload;
* **authentication and integrity**, allowing the receiver to detect modification or forgery of protected data.

The cryptographic protection is applied at the BTP message level so that the protocol does not rely exclusively on transport-specific security.

Cryptographic algorithms, nonce construction, authenticated fields, and validation rules are defined by the encryption specification.

### 3.10 Deterministic validation
...
Authenticated encryption protects the confidentiality and integrity of application payloads, but it does not constitute a complete wireless or network security system.

BTP does not by itself prevent:
...
* key compromise;
* replay when anti-replay protection is not provided.

Some protocol metadata may also remain visible even when the payload is encrypted.

Systems requiring stronger properties must provide them through the surrounding architecture.
...
* wireless communication where message-level protection is required;
...
BTP does not provide a complete network-security infrastructure.
...
remain responsibilities of the surrounding system.
...
* protection of application data over exposed communication channels;
...
When the underlying communication medium cannot be considered trusted, BTP can additionally protect application payloads using authenticated encryption.

The protocol focuses on framing, message identity, serialization, fragmentation, traffic classification, discovery, integrity, and optional cryptographic protection.

Routing, provisioning, large-scale streaming, RF availability, and complete network security remain outside its scope.
