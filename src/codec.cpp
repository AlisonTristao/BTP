...
// CRC-32/ISO-HDLC (the reflected, poly-0xEDB88320 variant crc32() below has
// always computed), table-driven instead of bit-serial. The two are the exact
// same algorithm -- crc32(check, sizeof(check)) below still has to land on
// 0xCBF43926 for the ASCII string "123456789", the standard check value --
// only the per-byte cost changes: one lookup + xor + shift instead of eight
// branchless bit steps. This runs on every encode() and every decode(), so it
// is the one place in the library where a table beats a loop -- the loop's
// per-bit accumulator recurrence is a true data dependency -O3 cannot thread
// through a lookup table on its own; that rewrite has to happen here.
// 1 KiB of .rodata (flash on a microcontroller, not heap), generated once by
// the standard bit-reversed construction and never touched again.
const std::uint32_t kCrc32Table[256] = {
...
std::uint32_t crc32(const std::uint8_t* data, std::size_t size) noexcept {
    ...
    for (std::size_t index = 0U; index < size; ++index) {
        const std::uint8_t table_index =
            static_cast<std::uint8_t>(accumulator ^ data[index]);
        accumulator = kCrc32Table[table_index] ^ (accumulator >> 8U);
    }
